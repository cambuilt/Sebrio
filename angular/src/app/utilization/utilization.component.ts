...
export class UtilizationComponent implements OnInit, AfterViewInit {
	columns = {
		table: [
			{ id: 1, name: 'Users', dropdown: false, array: [], model: 'Users', dateTime: false, checked: true },
			{ id: 2, name: 'Role', dropdown: false, array: [], model: 'Role', dateTime: false, checked: true },
			{ id: 3, name: 'Logins', dropdown: false, array: [], dateTime: true, model: 'Logins', checked: true }
		], display: [], filter: []
	};

	columnStorageName = 'UTILIZATION_MAINTENANCE';
	...
	constructor(private utilsService: UtilsService, private activeEntitiesService: ActiveEntitiesService, private utilizationService: UtilizationService, public dialog: MatDialog, public translation: TranslationService) {
	...
	runFilter(filter) {
		if (this.utilsService.checkOnlineStatus()) {
			if (this.noFilter === false) {
				this.utilizationService.searchUtilization(this.databaseFilter).subscribe(response => {
	...
	desktopPrint() {
		if (this.utilsService.checkOnlineStatus()) {
			...
			this.utilizationService.printUtilization(this.databaseFilter).subscribe(response => {
