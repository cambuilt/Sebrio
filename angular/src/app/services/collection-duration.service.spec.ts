import { CollectionDurationService } from './collection-duration.service';
...
let collectionDurationService: CollectionDurationService;

describe('CollectionDurationService', () => {
	let mockRouter;
	let mockMatDialog;
	...
	beforeEach(() => {
		...
		TestBed.configureTestingModule({
			...
			providers: [BluetoothSerial, AuthService, CollectionDurationService,
				{ provide: Router, useValue: mockRouter },
				{ provide: MatDialog, useValue: mockMatDialog }]
		});

		authService = TestBed.get(AuthService);
		collectionDurationService = TestBed.get(CollectionDurationService);
	});
	...
	it('should search collection durations', done => {
		...
		collectionDurationService.searchDurations(JSON.stringify(filterBody)).subscribe(response => {
			expect(response.status).toBe(200);
			done();
		}, error => {
			fail(error);
