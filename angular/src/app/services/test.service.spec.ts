...
describe('TestService', () => {
	...
	beforeEach(() => {
		...
		TestBed.configureTestingModule({
			...
			providers: [BluetoothSerial, AuthService, TestService,
	...
	it('should create test', done => {
		...
		const body = {
			ChargeAmount: '50.00', Code: 'R56', DefaultContainer: '2GN',
			Description: 'R56', Destination: 'Blood Bank', HandlingInstructions: 'with care',
			IsActive: true, Lab: 'RHODES', LabDepartment: '', Volume: '123ml'
		};
		testService.createTest(JSON.stringify(body)).subscribe(response => {
			expect(response.status).toBe(200);
			done();
		}, error => {
			fail(error);
	...
	it('should get, edit and delete test', done => {
		authService.currentUser.role = 'RMP_TSA';  // Somehow, role gets lost.
		testService.getTests().subscribe(response => {
			expect(response.status).toBe(200);
			const groups = response.json();
			const id = groups[groups.length - 1]['Id'];
			const body = {
				ChargeAmount: '70.00', Code: 'R56', DefaultContainer: '2GN',
				Description: 'R56 Unit', Destination: 'Blood Bank', HandlingInstructions: 'with TLC',
				IsActive: true, Lab: 'RHODES', LabDepartment: '', Volume: '183ml'
			};
			testService.updateTest(id, JSON.stringify(body)).subscribe(responseUpdate => {
				expect(responseUpdate.status).toBe(200);
				testService.deleteTest(id).subscribe(responseDelete => {
